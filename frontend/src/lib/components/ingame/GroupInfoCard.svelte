<script lang="ts">
    ...
    import { groupsStore, stationsStore } from "../../stores";

    let selected: number = $state(0);
    let group = $derived($groupsStore[selected]);
    let stations = $derived(group ? group.stations.map(idx => $stationsStore[idx]) : []);
</script>
